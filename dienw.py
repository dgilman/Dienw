...
r_name = re.compile(r'^[A-Za-z0-9-=?&_]+$')
dstr = '%Y-%m-%d %H:%M:%S'
...
def link_hook(parser_env, namespace, body):
    (article, pipe, text) = body.partition('|')
    href = article.strip().replace(' ', '_')
...
def get(name):
    ...
    if not commit and not exists("%s.txt" % name):
        ...
        s = 'Status: 404 Not Found\r\n'
        s += html(pageTitle(name), content(pageTitle(name), c))
        return s
    ...
    if commit:
        ...
        if s.startswith("# "):
            ...
            c = '**See the [[%s|current version]].**' % name
            s = s.split('\n\n', 1)
            s = "%s\n\n%s\n\n%s" % (s[0], c, s[1])
    ...
    #the new pageTitle() always returns something, it formats strings.
    #i'm not really sure what this was supposed to catch.  commenting it out
    #seems to fix things.
...
def info(name):
    ...
    if not commit:
        ...
        for link in inboundLinks(name[:-4]):
            if exists("%s.txt" % link):
                inbound[link] = pageTitle(link)
            else:
                inbound[link] = link
        outbound = {}
        for link in outboundLinks(name[:-4]):
            if exists("%s.txt" % link):
                outbound[link] = pageTitle(link)
            else:
                outbound[link] = link
    history = git_log(name)
    t = "Info on [[%s|%s]]" % (page_name, pageTitle(page_name))
    s = "==Statistics==\n\n"
    if not commit:
        s += ";Last edited by\n"
        s += ":    %s\n" % history[0]['author']
    ...
    for entry in history[cut:]:
        ...
        s += "; Date\n"
        if cc:
            ds = ": UNTESTED   **[%s](/%s?commit=)**\n"
        else:
            ds = ": %s\n"
        s += ds % (entry['atime'].strftime(dstr))
        s += "; Compare \n"
        if not cc and commit:
            s += ": [%s current with this]\n" % \
                 "/@diff/%s?commit=%s&commit=%s" % \
                 (name[:-4], commit[0], entry['commit'])
        if not entry == history[0]:
            s += ": [%s this with latest]\n" % \
                "/@diff/%s?commit=%s" % (name[:-4], entry['commit'])
        s += "\n\n"
    return html(t, content(markdown(t), markdown(s)))

def meta(name):
    if name == 'about':
        t = "About This Site"
        c = "This is a [http://dienw.com/wiki Dienw] based wiki. "
        c += "Dienw is based on [http://infomesh.net/pwyky/ pwyky], "
        c += "[http://blitiri.com.ar/git/?p=wikiri wikiri] and "
        c += "[http://git-scm.com/ git]. "
        c += "To find out more about it, please consult the "
        c += "[http://dienw.com/wiki#documentation Documentation].\n\n"
        ...
        p.sort()
        c += '\n'.join(['* [[@meta/%s| %s]] - %s' % (k, k, v) for k, v in p])
       ...
       results.sort()
       results = ['* [[%s|%s]]' % (fn, pageTitle(fn)) for fn in results]
       t = 'All Pages'
       ...
       return html(t, content(t, markdown(c)))
        ...
        for key in keys:
            ...
            fp.sort()
            c += '* [[%s|%s]] from: ' % (key, pageTitle(key))
            c += ', '.join(['[[%s|%s]]\n' % (v, pageTitle(v)) for v in fp])
        if len(keys) == 0:
